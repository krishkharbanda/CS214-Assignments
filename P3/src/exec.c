...
int execute_builtin(const char *cmd, char **argv, int argc) {
    ...
    if (strcmp(cmd, "which") == 0) {
        ...
        // Fail if the argument is a built-in
    ...
    if (strcmp(cmd, "exit") == 0) {
        return 0; // Handled in shell_loop
    }
    ...
    if (strcmp(cmd, "die") == 0) {
        ...
        if (argc > 1) printf("\n");
        return -1; // Special return code to signal termination
    }
...
int execute_job(Job *job, int last_exit_status, int *new_exit_status, int is_interactive) {
    ...
    for (int i = 0; i < job->segment_count; i++) {
        ...
        if (pid == 0) {
            ...
            if (i == 0 && !is_interactive && seg->input_file == NULL) {
                int devnull_fd = open("/dev/null", O_RDONLY);
                if (devnull_fd < 0) {
                    perror("/dev/null");
                    exit(EXIT_FAILURE);
